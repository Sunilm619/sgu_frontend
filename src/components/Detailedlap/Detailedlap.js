...
function Detailedlap() {
    let navigate = useNavigate();
    const onSaleRef = useRef(null);
    const { laparryy, setaddtocartlist } = Store();
    ...
    let { id } = useParams();
    let url = `http://localhost:4000/auth/productslaptop/${id}`;
    console.log(laparryy)

    useEffect(() => {
        // console.log(laparryy)
        const filtrdarry = laparryy.filter(e => e._id !== id);
                ...
                <div className='container'>
                    <div className='row'>
                        <div className='col-md-5'>
                            {prodlist.imgurl && <img src={prodlist.imgurl} alt={prodlist.description} />}
                        </div>
                        <div className='col-md-7'>
                            <h6 className='font-bold'>{prodlist.description}</h6>
                            <div className='flex'>
                                <div className='text-yellow-200 flex'>
                                    {Array.from({ length: prodlist.ratingcount || 0 }).map((_, index) => (
                                        <Star key={index} />
                                    ))}
                                </div>
                                <p className='gap-4 mx-4'>({prodlist.reviewcount || 0} Reviews) </p>
                                <p className='font-bold'>{prodlist.licontent}</p>
                            </div>
                            <hr />
                            <div className='flex'>
                                <h3 className='font-bold'>₹{prodlist.price}</h3>
                                <h3 className='line-through font-bold text-gray-400 ml-2'>₹100000</h3>
                            </div>
                            ...
                            <div className='row flex'>
                                <div className='col-md-3 border-2 flex rounded-2xl justify-around items-center'>
                                    <button onClick={() => { setNum(Math.max(0, num - 1)) }}>-</button>
                                    <span>{num}</span>
                            ...
                            <div className='flex justify-between my-3'>
                                <div className='flex gap-2'>
                                    <CiHeart className="text-2xl" /><span>Add Wishlist</span>
                                    <GoStack className="text-2xl" /><span>Add Compare</span>
                                </div>
                                <div className='flex'>
                                    <CiShare2 className="text-2xl" /><span>Share</span>
                                </div>
                            </div>
                            <hr />
                        </div>
                ...
                <div>
                    <div className='flex justify-center container-fluid'>
                        <ul className='inline-flex space-x-20'>
                            <li
                                ...
                                className={`cursor-pointer ${description ? "text-orange-500 text-2xl underline" : ""}`}
                            >
                                Description
                            </li>

                            <li className={`cursor-pointer ${addtnlinfo ? "text-orange-500 text-2xl underline" : ""}`}
                                onClick={() => {
                            ...
                            <li className={`cursor-pointer ${shippin ? "text-orange-500 text-2xl underline" : ""}`}
                                onClick={() => {
                                    setDescription(false);
                                    setReviw(false);
                                    setShippin(true);
                            ...
                            <li className={`cursor-pointer ${reviw ? "text-orange-500 text-2xl underline" : ""}`}
                                onClick={() => {
                                    setDescription(false);
                                    setReviw(true);
                                    setShippin(false);
                    ...
                    {description && (
                        <div>
                            ...
                            <img src={prodlist.broadimg} alt="im" />
                        ...
                        {relatedProducts.map((product) => (
                            <div key={product._id} className='col-md-3 mb-4'>
                                <div className='border p-3 rounded'>
                                    <img src={product.imgurl} alt={product.description} className='img-fluid' />
                                    ...
                                    <button
                                        ...
                                        onClick={() => setaddtocartlist({ ...product, quantity: 1 })}
                                    >
                                        Add to Cart
                                    </button>
                                </div>
                    ...
                    <div className='d-flex overflow-auto' ref={onSaleRef}>
                        {filted.map((item) => (
                            <div key={item._id} className='card m-2'>
                                <img src={item.imgurl} alt={item.description} className='w-[100px] card-img-top' />
                                <div className='card-body'>
                                    <h5 className='card-title'>{item.description}</h5>
                                    <p className='card-text'>₹{item.price}</p>
                                    <div className='flex flex-col'>
                                        <button className='btn btn-primary' onClick={() => setaddtocartlist({ ...item, quantity: 1 })}>
                                            Add
                                        </button>
                                    </div>
