...
export default function Header() {
    ...
    const search = async () => {
        ...
        const response = await fetch(`http://localhost:4000/auth/findproduct?search=${searchvalue}`, {
            method: 'GET',
            headers: {
    ...
    return (
        <div className='container-fluid text-secondary'>
            <div className='row'>
                <div className='flex md:items-center justify-between md:h-[40px]' style={{ backgroundColor: '#232f3f' }}>
                    ...
                    <div className='col-md-4 hidden md:block text-center'>
                        <p>
                            Sign up for 10% off your first order: <a href='/' className='text-light'>Sign up</a>
                        </p>
                    </div>
                    ...
                    <div className='col-md-4 flex justify-end max-sm:pt-3 md:pt-0 '>
                        {user !== 'Login' ? (
                            <button onClick={handleSignOut} className='btn btn-outline-light'>Sign Out</button>
                        ) : (
                            <button onClick={handleLoginClick} className='btn btn-outline-light'>Sign In</button>
                        )}
                    </div>
                </div>

                <div className='flex items-center' style={{ backgroundColor: '#232f3f' }}>
                    ...
                    <div className='col-md-5 hidden md:block'>
                        <div className='input-group rounded-pill bg-white'>
                            ...
                            <input ref={searchval}
                                className="form-control" type="search" name="q" placeholder="Search for products ..." />
                            <button
                                            ...
                                            {searchproducts.map((product, index) => (
                                                <li key={index} className='p-2 border-bottom'>
                                                    <div className='d-flex align-items-center'>
                                                        <img src={product.imgurl} alt={product.name} style={{ width: '50px', height: '50px', marginRight: '10px' }} />
                                                        <div>
                                                            <h6>{product.title}</h6>
                                                            <p>${product.price}</p>
                                                        </div>
                    ...
                    <div className='col-md-4  flex justify-between md:justify-around md:items-center'>
                        <div className='text-2xl md:hidden' onClick={toggleMenu} style={{ cursor: 'pointer' }}>
                            <TiThMenu />
                        </div>
                        ...
                        <div className='flex items-center'>
                            ...
                            <span className='text-orange-400 bg-slate-200 p-1 rounded-3xl'>{totalQuantity}</span>
            ...
            {isLoginModalOpen && <LoginRegiste closed={() => setIsLoginModalOpen(false)} />}
        </div >
    );
}
